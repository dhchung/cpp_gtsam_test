...
OpenglPointCloud::OpenglPointCloud(){
...
void OpenglPointCloud::framebuffer_size_callback(GLFWwindow* window, int width, int height) { // Change viewport if the user change the size of the window
    glViewport(0, 0, width, height);
}

void OpenglPointCloud::processInput(GLFWwindow * window) {
    if(glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) {
        glfwSetWindowShouldClose(window, true);
    }
}

int OpenglPointCloud::InitializeOpenGL(){
...
void OpenglPointCloud::show_all_point_cloud(){

}

void OpenglPointCloud::show_point_cloud(PointCloud & pt_cld){
